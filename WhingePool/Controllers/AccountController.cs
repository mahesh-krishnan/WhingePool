...
namespace WhingePool.Controllers
{
    ...
    public class AccountController : Controller
    {
        public AccountController()
            : this(IdentityConfig.Secrets,
                   IdentityConfig.Logins,
                   IdentityConfig.Users,
                   IdentityConfig.Roles) {}

        public AccountController(IUserSecretStore secrets,
                                 IUserLoginStore logins,
                                 IUserStore users,
                                 IRoleStore roles)
        {
            Secrets = secrets;
                                              ...
                                              string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (await Secrets.Validate(model.UserName,
                                           model.Password))
                {
                    var userId = await Logins.GetUserId(IdentityConfig.LocalLoginProvider,
                                                        model.UserName);
                    await SignIn(userId,
                                 model.RememberMe);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Disassociate(string loginProvider,
                                                     string providerKey)
        {
            ManageMessageId? message = null;
            var userId = User.Identity.GetUserId();
            if (await UnlinkAccountForUser(userId,
                                           loginProvider,
                                           providerKey))
            {
        ...
        private async Task<bool> UnlinkAccountForUser(string userId,
                                                      string loginProvider,
                                                      string providerKey)
        {
            var ownerAccount = await Logins.GetUserId(loginProvider,
                                                      providerKey);
            if (ownerAccount == userId)
            {
                if ((await Logins.GetLogins(userId)).Count > 1)
                {
                    await Logins.Remove(userId,
                                        loginProvider,
                                        providerKey);
                    return true;
                }
        ...
        private async Task<bool> ChangePassword(string userName,
                                                string oldPassword,
                                                string newPassword)
        {
            var changePasswordSucceeded = false;
            if (await Secrets.Validate(userName,
                                       oldPassword))
            {
                changePasswordSucceeded = await Secrets.UpdateSecret(userName,
                                                                     newPassword);
            }
        ...
        private async Task SignIn(string userId,
                                  IEnumerable<Claim> claims,
                                  bool isPersistent)
        {
            var user = await Users.Find(userId) as User;
            if (user != null)
            {
                // Replace UserIdentity claims with the application specific claims
                var userClaims = IdentityConfig.RemoveUserIdentityClaims(claims);
                IdentityConfig.AddUserIdentityClaims(userId,
                                                     user.UserName,
                                                     userClaims);
                IdentityConfig.AddRoleClaims(await Roles.GetRolesForUser(userId),
                                             userClaims);
                IdentityConfig.SignIn(HttpContext,
                                      userClaims,
                                      isPersistent);
            }
        }
        ...
        private class ChallengeResult : HttpUnauthorizedResult
        {
            ...
            {
                context.HttpContext.Challenge(LoginProvider,
